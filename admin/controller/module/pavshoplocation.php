<?php
class ControllerModulePavshoplocation extends Controller {
	private $error = array();
	...
	public function initData() {
		$this->language->load('module/pavshoplocation');
		...
		$this->load->model('pavshoplocation/location');

		$model = $this->model_pavshoplocation_location;

		$model->installModule();

		$this->data['heading_title'] = $this->language->get('heading_title');

		$this->data['text_location_option_open'] = $this->language->get('text_location_option_open');
		$this->data['text_location_option_close'] = $this->language->get('text_location_option_close');
		...
		$menus = array();
		$menus["modules"] = array("link"=>$this->url->link('module/pavshoplocation', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_manage_modules'));
		$menus["grouplocation"] = array("link"=>$this->url->link('module/pavshoplocation/grouplocation', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_group_location_map'));
		$menus["location"] = array("link"=>$this->url->link('module/pavshoplocation/location', 'token=' . $this->session->data['token'], 'SSL'),"title"=>$this->language->get('menu_location_map'));

		$this->data["menus"] = $menus;

		$this->document->addStyle('view/stylesheet/pavshoplocation.css');
	}
	...
	public function setBreadcrumb() {
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/pavshoplocation', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	}
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('pavshoplocation', $this->request->post);
			...
			if ($action == 'save-stay') {
				$this->redirect($this->url->link('module/pavshoplocation', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
				$this->redirect($this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}

		$this->data['action'] = $this->url->link('module/pavshoplocation', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['modules'] = array();

		if (isset($this->request->post['pavshoplocation_module'])) {
			$this->data['modules'] = $this->request->post['pavshoplocation_module'];
		} elseif ($this->config->get('pavshoplocation_module')) { 
			$this->data['modules'] = $this->config->get('pavshoplocation_module');
		}
		...
		$this->template = 'module/pavshoplocation/module.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function grouplocation() {
		...
		$this->data['token'] = $this->session->data['token'];
		$this->data['insert_link'] = $this->url->link('module/pavshoplocation/groupinsert', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['action'] = $this->url->link('module/pavshoplocation/groupdelete', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$location_total = $this->model_pavshoplocation_location->getTotalGroupLocations($data);
		$results = $this->model_pavshoplocation_location->getGroupLocations($data);
		foreach ($results as &$result) {
			$result['action'] = $this->url->link('module/pavshoplocation/groupupdate', 'group_id='.$result['group_location_id'].'&token=' . $this->session->data['token'] . $url, 'SSL');;
		}
		...
		$this->data['sort_name'] = $this->url->link('module/pavshoplocation/grouplocation', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('module/pavshoplocation/grouplocation', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('module/pavshoplocation/grouplocation', $url.'&token=' . $this->session->data['token'], 'SSL');
		}
		$this->data['stores'] = $stores;
		...
		$this->template = 'module/pavshoplocation/group_location.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function groupinsert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_pavshoplocation_location->addGroupLocation($this->request->post);

			$this->redirect($this->url->link('module/pavshoplocation/grouplocation', 'token=' . $this->session->data['token'], 'SSL'));
		}
		$this->getGroupFrom();
	}
	...
	public function groupupdate() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_pavshoplocation_location->editGroupLocation($this->request->get['group_id'], $this->request->post);

			if ($action == 'save-stay') {
				$this->redirect($this->url->link('module/pavshoplocation/groupupdate', 'group_id='.$this->request->get['group_id'].'&token=' . $this->session->data['token'], 'SSL'));
			} else {
				$this->redirect($this->url->link('module/pavshoplocation/grouplocation', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
	...
	public function groupdelete() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_pavshoplocation_location->deleteGroupLocation($this->request->post['selected']);
	...
	public function getGroupFrom() {
		...
		if (!isset($this->request->get['group_id'])) {
			$this->data['action'] = $this->url->link('module/pavshoplocation/groupinsert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('module/pavshoplocation/groupupdate', 'token=' . $this->session->data['token'] . '&group_id=' . $this->request->get['group_id'] . $url, 'SSL');
		}
		$this->data['cancel'] = $this->url->link('module/pavshoplocation/grouplocation', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['group_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$grouplocation_info = $this->model_pavshoplocation_location->getGroupLocation($this->request->get['group_id']);
		}
		...
		$this->template = 'module/pavshoplocation/group_form.tpl';
	...
	public function location() {
		...
		$this->data['token'] = $this->session->data['token'];
		$this->data['insert_link'] = $this->url->link('module/pavshoplocation/insert', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['action'] = $this->url->link('module/pavshoplocation/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');

		$location_total = $this->model_pavshoplocation_location->getTotalLocations($data);
		$results = $this->model_pavshoplocation_location->getlocations($data);
		foreach ($results as &$result) {
			$result['action'] = $this->url->link('module/pavshoplocation/update', 'location_id='.$result['location_id'].'&token=' . $this->session->data['token'] . $url, 'SSL');;
		}
		...
		$this->data['sort_title'] = $this->url->link('module/pavshoplocation/location', 'token=' . $this->session->data['token'] . '&sort=title' . $url, 'SSL');
		...
		$pagination->limit = $this->config->get('config_admin_limit');
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('module/pavshoplocation/location', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		foreach ($stores as &$store) {
			...
			$store['option'] = $this->url->link('module/pavshoplocation/location', $url.'&token=' . $this->session->data['token'], 'SSL');
		}
		$this->data['stores'] = $stores;
		...
		$this->template = 'module/pavshoplocation/location.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function update () {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_pavshoplocation_location->editLocation($this->request->get['location_id'], $this->request->post);

			if ($action == 'save-stay') {
				$this->redirect($this->url->link('module/pavshoplocation/update', 'location_id='.$this->request->get['location_id'].'&token=' . $this->session->data['token'], 'SSL'));
			} else {
				$this->redirect($this->url->link('module/pavshoplocation/location', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_pavshoplocation_location->addLocation($this->request->post);

			$this->session->data['success'] = $this->language->get('text_success');
			if ($action == 'save-stay') {
				$this->redirect($this->url->link('module/pavshoplocation/insert', 'token=' . $this->session->data['token'], 'SSL'));
			} else {
				$this->redirect($this->url->link('module/pavshoplocation/location', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
	...
	public function delete() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_pavshoplocation_location->deleteLocation($this->request->post['selected']);
	...
	public function getForm() {
		...
		$groups = $this->model_pavshoplocation_location->getGroupLocations(null);
		$this->data['groups'] = $groups;

		$url = '';

		if (!isset($this->request->get['location_id'])) {
			$this->data['action'] = $this->url->link('module/pavshoplocation/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('module/pavshoplocation/update', 'token=' . $this->session->data['token'] . '&location_id=' . $this->request->get['location_id'] . $url, 'SSL');
		}
		$this->data['cancel'] = $this->url->link('module/pavshoplocation/location', 'token=' . $this->session->data['token'] . $url, 'SSL');

		if (isset($this->request->get['location_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		$location_info = $this->model_pavshoplocation_location->getLocation($this->request->get['location_id']);
    	}
		...
		if (isset($this->request->post['location_option'])) {
			$this->data['option'] = $this->request->post['location_option'];
		} elseif (!empty($location_info)) {
			$this->data['option'] = $location_info['option'];
		} else {
			$this->data['option'] = '';
		}
		...
		$this->template = 'module/pavshoplocation/location_form.tpl';
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/pavshoplocation')) {
