	...
	public function generate($orders, $mode = 'display', $type = 'invoice', $lang = '') {
		...
		*/
		...
		if ($order_info) {
			...
			{
				$languages = array();
				foreach(MijoShop::get('db')->getOcLanguages() as $l) {
					$languages[$l['language_id']] = $l;
				}
				if($lang) {
					$user_lang =$languages[$lang];
				} else {
					$user_lang = $languages[$order_info['language_id']];
				}
			}
			else
			{
			...
			$template->data = array_merge($template->data, array(
				...
				'invoice_prefix'         => $order_info['invoice_prefix'],
					...
					'date_due'         	=> ($this->config->get('pdf_invoice_duedate') != '') ? date($language->get('date_format_short'), strtotime($order_info['date_added'].' + '.$this->config->get('pdf_invoice_duedate').' day')) : '',
